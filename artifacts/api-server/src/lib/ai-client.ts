 ...
 *
 * The @google/genai SDK (v1+) supports both modes through the same constructor.
 */
...
function createClient(): { ai: GoogleGenAI; provider: Provider; model: string } {
  ...
  if (geminiBase && geminiKey) {
    ...
    logger.info(
      { provider: "gemini-proxy", model },
      "AI provider: Replit Gemini proxy",
    );
  ...
  throw new Error(
    [
      ...
      "  • Local dev  — provision the Replit Gemini integration (sets AI_INTEGRATIONS_GEMINI_BASE_URL + AI_INTEGRATIONS_GEMINI_API_KEY automatically)",
      "  • Production — set VERTEX_AI_PROJECT_ID (+ optional VERTEX_AI_LOCATION, VERTEX_AI_MODEL)",
    ].join("\n"),
  );
}

const { ai, provider, model: defaultModel } = createClient();

export { ai, provider, defaultModel as model };
